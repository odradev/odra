...
macro_rules! assert_events {
    ($contract:ident, $ ( $event_ty:ty ),+ ) => {
        ...
        $(
            __idx += 1;
            let __ev = odra_env::test_utils::get_event::<$event_ty>(&$contract.address(), __idx).unwrap();
            let __name = stringify!($event_ty).to_string();
            let __name = __name.split("::").last().unwrap();
            assert_eq!(
                <$event_ty as odra_types::event::Event>::name(&__ev), __name
            );
        )+
    };
    ($contract:ident, $ ( $event:expr ),+ ) => {
        ...
        $(
            __idx += 1;
            let __ev = odra_env::test_utils::get_event(&$contract.address(), __idx).unwrap();
            assert_eq!(
                $event, __ev
            );
        )+
