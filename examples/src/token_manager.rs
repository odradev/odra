...
mod test {
    ...
    fn minting_works() {
        ...
        contract.mint(plascoin, &user1, &pls_balance1);
        contract.mint(plascoin, &user2, &pls_balance2);
        contract.mint(plascoin, &user3, &pls_balance3);

        contract.mint(my_coin, &user1, &mcn_balance1);
        contract.mint(my_coin, &user2, &mcn_balance2);
        contract.mint(my_coin, &user3, &mcn_balance3);

        assert_eq!(
            contract.balance_of(plascoin, &user1),
            pls_balance1
        );
        assert_eq!(
            contract.balance_of(plascoin, &user2),
            pls_balance2
        );
        assert_eq!(
            contract.balance_of(plascoin, &user3),
            pls_balance3
        );
        assert_eq!(
            contract.balance_of(my_coin, &user1),
            mcn_balance1
        );
        assert_eq!(
            contract.balance_of(my_coin, &user2),
            mcn_balance2
        );
        assert_eq!(
            contract.balance_of(my_coin, &user3),
            mcn_balance3
        );
    }
