...
mod test {
    ...
    fn minting_works() {
        ...
        assert_eq!(
            contract.balance_of(plascoin.clone(), &user1),
            pls_balance1
        );
        assert_eq!(
            contract.balance_of(plascoin.clone(), &user2),
            pls_balance2
        );
        assert_eq!(
            contract.balance_of(plascoin.clone(), &user3),
            pls_balance3
        );
        assert_eq!(
            contract.balance_of(my_coin.clone(), &user1),
            mcn_balance1
        );
        assert_eq!(
            contract.balance_of(my_coin.clone(), &user2),
            mcn_balance2
        );
        assert_eq!(
            contract.balance_of(my_coin.clone(), &user3),
            mcn_balance3
        );
    }
