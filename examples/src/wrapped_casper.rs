...
mod tests {
    ...
    use super::{
        ...
        WrappedCasperDeployer, WrappedCasperRef
    };

    fn setup() -> (WrappedCasperRef, Address, Balance, Address, Balance) {
        let token: WrappedCasperRef = WrappedCasperDeployer::init();
    ...
    fn test_withdrawal() {
        ...
        let withdrawal_amount_balance: Balance = withdrawal_amount.to_balance().unwrap();
        token.withdraw(withdrawal_amount);

        // Then the user has the withdrawn tokens back.
        assert_eq!(test_env::token_balance(account), withdrawal_amount_balance);
    ...
    fn test_withdrawal_amount_exceeding_account_deposit() {
        test_env::assert_exception(odra_modules::erc20::errors::Error::InsufficientBalance, || {
            // Given a new contract.
