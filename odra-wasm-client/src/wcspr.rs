...
impl WCSPRClient {
        ...
        self.wasm_client
            .call_entry_point_with_proxy::<U256, casper_types::U256>(
                *self.address,
                "balance_of",
                runtime_args! {
                    "address" => IntoOdraValue::<odra_core::prelude::Address>::into_odra_value(address)?
                }
            )
            .await
    }
    ...
    #[wasm_bindgen]
    pub async fn deposit(&mut self, attached_value: U512) -> Result<(), JsError> {
        self.wasm_client
    ...
    #[wasm_bindgen]
    pub async fn withdraw(&mut self, amount: U256) -> Result<(), JsError> {
        self.wasm_client
            .call_entry_point(
                *self.address,
                "withdraw",
